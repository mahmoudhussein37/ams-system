...
<script>
    ...
    <c:if test="${not empty firstCounseling}">
        $(".detail-div").load("${baseUrl}/professor/studentGuidance/counseling/counselingDetail?counselingId=${firstCounseling.id}");
    </c:if>
    $("body").on('click', '.student-detail', function (e) {
        e.preventDefault();
        var counselingId = $(this).attr("data-counseling-id");
        $(".detail-div").load("${baseUrl}/professor/studentGuidance/counseling/counselingDetail?counselingId=" + counselingId);
