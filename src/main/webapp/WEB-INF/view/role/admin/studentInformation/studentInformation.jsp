<%@include file="/WEB-INF/view/include/topTag.jsp" %>
    <%@include file="/WEB-INF/view/include/head.jsp" %>
        <!--end::Head-->

        <!--begin::Body-->

        <body id="kt_body" class="header-fixed header-mobile-fixed page-loading">
            <%@include file="/WEB-INF/view/include/headerBar.jsp" %>
                <!--begin::Content-->
                <div class="content  d-flex flex-column flex-column-fluid" id="kt_content">
                    <!--begin::Entry-->
                    <div class="d-flex flex-column-fluid">
                        <!--begin::Container-->
                        <div class=" container ">
                            <div class="row">
                                <div class="col-md-12">
                                    <!--begin::Card-->
                                    <div class="card card-custom">
                                        <div class="card-header">
                                            <h3 class="card-title font-weight-bolder">
                                                <spring:message code="menu.admin.sub1_2" />
                                            </h3>

                                        </div>
                                        <div class="card-body">
                                            <!-- Row 1: Basic search fields -->
                                            <div class="row mb-4">
                                                <div class="col-md-3">
                                                    <label class="font-weight-bold text-muted small mb-1">
                                                        <spring:message code="common.studentsNumber" />
                                                    </label>
                                                    <input type="text" id="search-number"
                                                        class="form-control input-enter" value="" placeholder="..." />
                                                </div>
                                                <div class="col-md-4">
                                                    <label class="font-weight-bold text-muted small mb-1">
                                                        <spring:message code="common.studentsName" />
                                                    </label>
                                                    <input type="text" id="search-name" class="form-control input-enter"
                                                        value="" placeholder="..." />
                                                </div>
                                                <div class="col-md-5">
                                                    <label class="font-weight-bold text-muted small mb-1">
                                                        <spring:message code="common.department" />
                                                    </label>
                                                    <select id="search-division" class="form-control">
                                                        <option value="0">-
                                                            <spring:message code="common.all" /> -
                                                        </option>
                                                        <c:forEach var="division" items="${divisions}">
                                                            <option value="${division.id}">${division.name}</option>
                                                        </c:forEach>
                                                    </select>
                                                </div>
                                            </div>
                                            <!-- Row 2: Additional filters + action buttons -->
                                            <div class="row align-items-end">
                                                <div class="col-md-3">
                                                    <label class="font-weight-bold text-muted small mb-1">
                                                        <spring:message code="common.schoolYear" />
                                                    </label>
                                                    <select id="search-schoolYear" class="form-control">
                                                        <option value="0">-
                                                            <spring:message code="common.all" /> -
                                                        </option>
                                                        <option value="1">1</option>
                                                        <option value="2">2</option>
                                                        <option value="3">3</option>
                                                        <option value="4">4</option>
                                                        <option value="5">5</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-3">
                                                    <label class="font-weight-bold text-muted small mb-1">
                                                        <spring:message code="admin.accountStatus" />
                                                    </label>
                                                    <select id="search-accountStatus" class="form-control">
                                                        <option value="">-
                                                            <spring:message code="common.all" /> -
                                                        </option>
                                                        <option value="active">
                                                            <spring:message code="student.status.active" />
                                                        </option>
                                                        <option value="pending">
                                                            <spring:message code="student.status.pending" />
                                                        </option>
                                                        <option value="disabled">
                                                            <spring:message code="student.status.disabled" />
                                                        </option>
                                                    </select>
                                                </div>
                                                <div class="col-md-6 d-flex justify-content-end">
                                                    <button class="btn btn-primary mr-2" onclick="search()">
                                                        <i class="fa fa-search mr-1"></i>
                                                        <spring:message code="common.search" />
                                                    </button>
                                                    <button class="btn btn-light"
                                                        onclick="javascript:location.reload()">
                                                        <i class="fa fa-undo mr-1"></i>
                                                        <spring:message code="common.reset" />
                                                    </button>
                                                </div>
                                            </div>

                                            <br /><br />



                                            <div class="table-div">


                                            </div>





                                            <br /><br />
                                            <div class="separator separator-solid my-5"></div>
                                            <br /><br />
                                            <div class="detail-div">

                                            </div>

                                        </div>
                                    </div>
                                    <!--end::Card-->
                                </div>
                            </div>


                        </div>
                        <!--end::Container-->
                    </div>
                    <!--end::Entry-->
                </div>
                <!--end::Content-->

                <%@include file="/WEB-INF/view/include/footerBar.jsp" %>


                    <%@include file="/WEB-INF/view/include/userPanel.jsp" %>


                        <%@include file="/WEB-INF/view/include/footerScript.jsp" %>

                            <script>
                                function search() {
                                    var number = $("#search-number").val().trim();
                                    var name = $("#search-name").val().trim();
                                    var division = $("#search-division").children("option:selected").val().trim();
                                    var schoolYear = $("#search-schoolYear").children("option:selected").val().trim();
                                    var accountStatus = $("#search-accountStatus").children("option:selected").val().trim();

                                    var tableUrl = "${baseUrl}/admin/studentManagement/studentInformation/studentTable"
                                        + "?number=" + encodeURIComponent(number)
                                        + "&name=" + encodeURIComponent(name)
                                        + "&division=" + encodeURIComponent(division)
                                        + "&schoolYear=" + encodeURIComponent(schoolYear)
                                        + "&accountStatus=" + encodeURIComponent(accountStatus);
                                    $(".table-div").load(tableUrl);
                                }




                                $(document).ready(function () {
                                    $(".table-div").load("${baseUrl}/admin/studentManagement/studentInformation/studentTable");

                                    $(".input-enter").keydown(function (key) {
                                        if (key.keyCode == 13) {
                                            search();
                                        }
                                    });

                                    <c:if test="${not empty result}">
                                        alert("<spring:message code='common.success' javaScriptEscape="true" />");
                                        location.href="${baseUrl}/admin/studentManagement/studentInformation";
                                    </c:if>
                                });
                            </script>
        </body>

        </html>