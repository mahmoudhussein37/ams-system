...
public class EnumToIntTypeHandler<E extends Enum<E>> extends BaseTypeHandler<E> {

    private Class<E> type;

    public EnumToIntTypeHandler(final Class<E> type) {
        if (type == null) {
    ...
    public void setNonNullParameter(final PreparedStatement ps, final int i, final E parameter, final JdbcType jdbcType) throws SQLException {
        try {
            Method method = type.getMethod("toNumber");
    ...
    private E getEnum(final Integer value) throws SQLException {
        try {
            return (E) type.getMethod("fromNumber", Integer.class).invoke(null, value);
        } catch (NoSuchMethodException | SecurityException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
            throw new SQLException(e);
        }
    }
